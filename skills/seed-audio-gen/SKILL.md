...
The default `./seedaudio-output/` is **relative to your current working directory** — run from your project/working dir (or pass `-o` / `--output-dir`) so audio doesn't land inside the skill package folder. It is disposable scratch (gitignored). For content you want to keep or hand to a downstream step, pass an explicit destination, e.g. `--output-dir content/origin/<article>/assets/` (tracked content) or a `.skill-work/` scratch dir.

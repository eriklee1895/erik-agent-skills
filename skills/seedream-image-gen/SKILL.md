...
Validated scenarios: headline swap on existing posters, sofa material change + adding an animal, metal sculpture → transparent glass, multi-region multi-color simultaneous edits. For details see the Marker Editing Protocol section in [references/api-reference.md](references/api-reference.md).
...
- [Prompt engineering guide](references/prompt-engineering.md) — five prompt formulas (generic / text / marker / consistency / style), failure-mode checklist

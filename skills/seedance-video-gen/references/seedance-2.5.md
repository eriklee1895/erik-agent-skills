...
| ratio | 六档 + adaptive；省略实测 16:9 | 同样六档 + adaptive；**官方默认 adaptive**。CLI 文生仍默认 16:9 |
| 格式 | 仅 mp4 | **mp4 + mov**（mov = H.264 + yuv444p + PCM，后期友好） |
| 参考上限 | 图9+视3+音3（脚本 total≤12） | **图30+视10+音10，共50**；音/视频总时长 30s |
...
| web_search | ✅ 仅纯文本 | ✅ 同样 |
| 计费 | 约 28 元/百万 token（含视频输入）/ 46（无）；以控制台为准 | **更高**（公开价约 42 / 70）；无免费额度；开通门槛同 2.0（余额≥200 或资源包） |
...
| 延长 | ≥1 `reference_video` + 延长意图（向前/向后延长、延续、续写） | **ratio 必须 adaptive** |

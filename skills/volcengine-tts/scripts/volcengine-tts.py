...
List speakers (local table, no API call):
    ...
    uv run volcengine-tts.py --list-speakers --filter type=bigtts --sort heat
...
def query_speakers(
    ...
    *,
    ...
    """Filter/sort the local ListSpeakers snapshot. Same logic as seed-audio-gen."""
...
def load_speakers(path: Path = SPEAKERS_JSON) -> list[dict[str, Any]]:
    if not path.exists():
        die(f"speakers.json not found: {path}")
    return json.loads(path.read_text(encoding="utf-8"))


def parse_speaker_filters(raw_filters: list[str] | None) -> dict[str, str]:
    ...
    for f in raw_filters:
        k, _, v = f.partition("=")
        if not k or not v:
            die(f"invalid --filter {f!r}; expected key=value (scene=视频配音 / type=bigtts / lang=ja)")
        filters[k] = v
    return filters


def format_speaker_list(speakers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "name": s.get("name", ""),
...
def _list_speakers(args: argparse.Namespace) -> None:
    """Read the local catalog (no API call, no API key)."""
    speakers = load_speakers()
    filters = parse_speaker_filters(args.filter)
    result = query_speakers(speakers, filters=filters or None, sort_by=args.sort)
    out = format_speaker_list(result)
...
def main() -> None:
    ...
    parser.add_argument(
        "--list-speakers",
        action="store_true",
        help="List speakers from local table (no API call) and exit",
    )
    parser.add_argument(
        "--filter",
        action="append",
        help="Filter speakers: scene=视频配音 / type=bigtts / lang=ja (repeatable)",
    )
    parser.add_argument("--sort", choices=["heat"], help="Sort speakers by field")
